...
static void print_help(const char *argv0)
{
  fprintf(stderr, "Usage: %s\n\n"
          "Adapted for BCTF\n\n"
          "Options:\n"
          " -s  server host (default: %s)\n"
          " -p  server port (default: %d)\n"
          , argv0, SERVER_HOST, SERVER_PORT);
}

void connect_server() // for BCTF
{
  struct sockaddr_in sin;
  int fd = socket(AF_INET, SOCK_STREAM, 0); // fd should be 3
  sin.sin_family = AF_INET;
  inet_aton(server_host, &sin.sin_addr);
  sin.sin_port = htons(server_port);
  if (fd < 0 || connect(fd, (struct sockaddr *)&sin, sizeof sin) < 0)
    fatal_error_fmt("failed to connect to the server: %s", strerror(errno));
  //if (dup2(fd, 3) < 0)
  //  fatal_error_fmt("failed to dup2: %s", strerror(errno));
}
...
#else
  int opt;
  while ((opt = getopt(argc, argv, "htvs:p:")) != -1) {
    switch (opt) {
    case 'h':
      print_help(argv[0]);
      return 0;
